...
#Ideas: Might need to use mask for discriminator so that we get some condtional information about parts that are always empty
...
class Generator(nn.Module):
	def __init__(self, z_dim, imgdim, d=128):
		...
		self.deconv2 = nn.ConvTranspose2d(  d*8, d*4, 3, 2, 1) # To revert, change stride back to 2
	...
	def forward(self, input):
		...
		x = F.tanh(self.deconv5(x))				# To encourage sparsity
		return x
